...
class PatchEmbed(nn.Module):
    """
    2D Image to Patch Embedding，16*16的768个卷积核加展平操作
    输入224*224*3，输出197*768
    """
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        ...
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()#有norm_layer则操作否则不做任何操作
...
class Attention(nn.Module):
    def __init__(self,
        ...
        head_dim = dim // num_heads#每一个head对应的qkv的维度 v，96
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        self.num_tokens = 2 if distilled else 1#默认为1
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        act_layer = act_layer or nn.GELU#默认GELU

        self.patch_embed = embed_layer(img_size=img_size, patch_size=patch_size, in_c=in_c, embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches#分块的个数，14*14=196

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))#第一个1为batch维度，方便拼接，后面（1，768）就是一维向，可训练参数
        self.dist_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None#默认为none,在这个vit模型中用不上，不用管
        ...
        # Representation layer,这里用不上 ，对应模型图的pre_logits
    ...
    def forward_features(self, x):
        ...
        # [1, 1, 768] -> [B, 1, 768],复制操作
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)
        if self.dist_token is None:#默认第一个
...
def vit_pretrained(num_classes: int = 2):
    """
    ...
    weights_dict = torch.load('/media/user/Disk02/zyl/graph/vit_base_patch16_224_in21k.pth')
...
if __name__=="__main__":
    model=vit_pretrained(num_classes=2)
